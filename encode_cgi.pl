...
if(!defined $file_lock)
{
	print qq
	{
		<p>Enter up to 4096 characters!</p>
		<p>I recommend using up all available 4,096 available character spaces for maximum picture prettiness!</p>
		<br/>
		<form method='post' action=''>
			<input type='text' name='file_lock' />
			<br/><br/>
...
my $string;
$string .= $characters[rand @characters] for 1..12;

$image->write(file=>'output/' . $string . '.bmp') or die 'Cannot save $string.bmp: ', $image->errstr;

print "Text encoded, click <a href='output/$string.bmp'>here</a> to view picture.";